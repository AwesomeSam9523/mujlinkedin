	...
	<div class="modal">
		<div class="left">
			...
			<div class="no-club" v-else>
				Very few connections? Join these clubs:
		...
		<div class="right">
			...
			<div class="name">
				{{ userData?.displayName }} <span class="pronouns" v-if="userData.gender">({{ getUserPronouns(userData.gender) }})</span>
			</div>
			<div class="bio">
				{{ userData?.bio }}
			</div>
...
export default {
	...
	data() {
		return {
			clubsData: {
				...
				acm: { motto: 'Lets code together', name: 'Association for Computing Machinery', logo: '' },
				ieee: { motto: 'No code left untouched', name: 'Institute of Electrical and Electronics Engineers', logo: '' },
	...
	async created() {
		const { data } = await service.get('/user/clubs'); 
		// this.userClubs = data;
		console.log(data);
	},
	mounted() {
